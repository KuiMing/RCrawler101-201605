...
TWSE_margin <- function(year=2016, mons="09", day="26"){
  date=paste0(year,mons,day)
  url <- paste0("http://www.twse.com.tw/exchangeReport/MI_MARGN",
                "?response=html&date=",date,"&selectType=24")
#   res <- POST(url,
#               add_headers(
...
# With rvest, can be runned in R, but not in terminal
  ...
  colnames(tables) <- c("code", "name", "MNew",	"MRedemption",
                        "MOutstanding",	"MYesterdayRemain",
                        "MTodayRemain",	"MTodayLimit",
                        "SRedemption", "SNew",	"SOutstanding",
                        "SYesterdayRemain",	"STodayRemain",
                        "STodayLimit","difference","note")
